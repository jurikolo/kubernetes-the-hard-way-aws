...
for instance in controller-0 controller-1 controller-2; do
  ...
  echo ssh -i k8s-hard-way.id_rsa ubuntu@$external_ip
done
```
...
NOTE: previously the username was `kubernetes`
...
```sh
curl -k --cacert ca.crt https://${KUBERNETES_PUBLIC_ADDRESS}/version
```
